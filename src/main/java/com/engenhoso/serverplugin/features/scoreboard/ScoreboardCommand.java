...
public class ScoreboardCommand implements CommandExecutor, TabCompleter {
    ...
    public ScoreboardCommand(ScoreboardService scoreboardService) {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0 || args[0].equalsIgnoreCase("toggle")) {
            boolean visivel = scoreboardService.alternarScoreboard(jogador);
        ...
        if (args[0].equalsIgnoreCase("on")) {
            scoreboardService.setScoreboardVisivel(jogador, true);
            jogador.sendMessage("§aScoreboard ativado.");
            return true;
        }

        if (args[0].equalsIgnoreCase("off")) {
            scoreboardService.setScoreboardVisivel(jogador, false);
